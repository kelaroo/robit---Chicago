...
public class AutoAlbastru extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        switch (tsePosition) {
            ...
            case LEFT:
                t1 = drive.trajectoryBuilder(startPose)
                        .lineToConstantHeading(new Vector2d(T1_X_LEFT, T1_Y_LEFT))
                        .build();
            ...
            default:
                t1 = drive.trajectoryBuilder(startPose)
                        .lineToConstantHeading(new Vector2d(T1_X_RIGHT, T1_Y_RIGHT))
                        .build();
        ...
        for(int i = 0; i < 3 && opModeIsActive() && !isStopRequested(); i++) {
            ...
            while(!robit.cuva.isElementIn()) {
                ...
                if(timer.milliseconds() > TIMEOUT) {
                    ...
                    robit.intake.intakeReverse();
                    Trajectory pizdaPula = drive.trajectoryBuilder(drive.getPoseEstimate())
                        .back(5.0).build();
